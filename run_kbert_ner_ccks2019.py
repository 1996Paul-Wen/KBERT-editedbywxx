...
def main():
    ...
    # Load or initialize parameters of pretrained_model.
    if args.pretrained_model_path is not None:
        ...
        for n, p in list(model.named_parameters()):
            if 'gamma' not in n and 'beta' not in n:
                # 均值为0，标准差0.02随机初始化参数。带下划线_的操作表示是inplace操作
                p.data.normal_(0, 0.02)

    ######################
    # 4.Build sequence labeling model base on pretrained_model, and try to use multiple GPUs, and load sequence labeling model to specified device
    # Build sequence labeling model. 创建命名实体识别模型的实例对象
    # model = BertGruCrf(args, model)
    # model = BertLstmCrf(args, model)
    # model = BertCrf(args, model)
    # model = BertSoftmax(args, model)
    # model = BertLstm(args, model)
    model = BertGru(args, model)
    # model = BertSoftmaxCross(args, model)
    ...
    def evaluate(args, is_test):
        ...
        for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vm_ids_batch, tag_ids_batch, padding_mask_ids_batch) in enumerate(batch_loader_bywxx(batch_size, input_ids, label_ids, mask_ids, pos_ids, vm_ids, tag_ids, padding_mask_ids)):
            ...
            loss, _, pred, gold = model(input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vm_ids_batch, padding_mask_ids_batch, batch_true_max_len)
            # pred预测值， gold真实值
    ...
    # ### optimizer for bertbase model ###
    # if hasattr(model, "module"):
    #     # 分层lr wxx
    #     bert_params = list(map(id, model.module.embedding.parameters()))
    #     bert_params += list(map(id, model.module.encoder.parameters()))
    #     rest_params = list(filter(lambda x: id(x) not in bert_params, model.module.parameters()))
    #     optimizer_grouped_parameters = [
    #         {'params': rest_params, 'lr': args.learning_rate*500, 'weight_decay_rate': 1e-3},  # lr: 0.01
    #         {'params': model.module.embedding.parameters(), 'weight_decay_rate': 0.0},
    #         {'params': model.module.encoder.parameters(), 'weight_decay_rate': 0.0}
    #     ]
    # else:
    #     # 分层lr wxx
    #     bert_params = list(map(id, model.embedding.parameters()))
    #     bert_params += list(map(id, model.encoder.parameters()))
    #     rest_params = list(filter(lambda x: id(x) not in bert_params, model.parameters()))
    #     optimizer_grouped_parameters = [
    #         {'params': rest_params, 'lr': args.learning_rate * 500, 'weight_decay_rate': 1e-3},  # lr: 0.01
    #         {'params': model.embedding.parameters(), 'weight_decay_rate': 0.0},
    #         {'params': model.encoder.parameters(), 'weight_decay_rate': 0.0}
    #     ]

    # 设置优化器BertAdam, 比Adam更新的一种优化算法，加入了正则化项-weight_decay
    # A.对不同的params分配不同的weight_decay_rate
    # B.warmup则有助于减缓模型在初始阶段对mini-batch的提前过拟合现象，保持分布的平稳。因为在训练的开始阶段，模型权重迅速改变
    ...
    for epoch in range(1, args.epochs_num+1):
        ...
        for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vm_ids_batch, tag_ids_batch, padding_mask_ids_batch) in enumerate(batch_loader_bywxx(batch_size, input_ids, label_ids, mask_ids, pos_ids, vm_ids, tag_ids, padding_mask_ids)):
            ...
            loss, _, _, _ = model(input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vm_ids_batch, padding_mask_ids_batch, batch_true_max_len)
            ...
            # nn.utils.clip_grad_norm_(model.parameters(), max_norm=20, norm_type=2.0)  # clip norm
            optimizer.step()
